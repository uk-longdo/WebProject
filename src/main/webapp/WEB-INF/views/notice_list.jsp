            	...
            	<div class="listmain">
            		...
            		<table class="listtable">
            			<tr>
      					...
      					<c:forEach var="ndata" items="${result}">
                     	<tr onclick="location.href='notice_read?idx=${ndata.notice_idx}'" >
                        	<td>${ndata.notice_idx}</td>
                        	<td>${ndata.notice_title} </td>
                        	<td>${ndata.user_id}</td>
                        	<td>${ndata.notice_viewcnt}</td>
                        	<td>${ndata.notice_credate}</td>
                     	</tr>
                     	</c:forEach>
      				</table>
      				<div class="paging">
			        ...
			        <c:choose>
			         <c:when test="${i eq pageNum}">
      				...
      				<div class="buttons">
      					<c:choose>
      					<c:when test="${sessionScope.loginid ne null && sessionScope.loginidgrade eq 6}">
      					<input class="insertbutton" type="button" value="공지작성" onclick="location.href='notice_insert'">
      					</c:when>
      					</c:choose>
      				</div>
				</div>
